export class Schema {

    public static Product2:Array<string> = [
        'Name',
        'ProductCode',
        'Description',
        'IsActive',
        'Family',
        'CurrencyIsoCode',
        'ExternalDataSourceId',
        'ExternalId',
        'DisplayUrl',
        'QuantityUnitOfMeasure',
        'StockKeepingUnit',
        'enxCPQ__Network_Availability_Strategy__c',
        'enxCPQ__Product_Feasibility_Strategy__c',
        'enxCPQ__Access_Medium__c',
        'enxCPQ__Billing_Frequency__c',
        'enxCPQ__Bundle_Configuration_Method__c',
        'enxCPQ__Charge_Criteria__c',
        'enxCPQ__Charge_Item_Action__c',
        'enxCPQ__Charge_Model__c',
        'enxCPQ__Charge_Name__c',
        'enxCPQ__Charge_Type__c',
        'enxCPQ__Charge_Editable_in_Runtime__c',
        'enxCPQ__Column_Dimension__c',
        'enxCPQ__Column_Numeric__c',
        'enxCPQ__Column_Value__c',
        'enxCPQ__Cover_Image_URL__c',
        'enxCPQ__Current_Inventory__c',
        'enxCPQ__Current_Lead_Time__c',
        'enxCPQ__Description_CN__c',
        'enxCPQ__Description_DE__c',
        'enxCPQ__Description_EN__c',
        'enxCPQ__Description_ES__c',
        'enxCPQ__Description_FR__c',
        'enxCPQ__Description_IT__c',
        'enxCPQ__Description_PL__c',
        'enxCPQ__Description_Pattern__c',
        'enxCPQ__Dimension_1_Numeric__c',
        'enxCPQ__Dimension_1_Value__c',
        'enxCPQ__Dimension_1__c',
        'enxCPQ__Dimension_2_Numeric__c',
        'enxCPQ__Dimension_2_Value__c',
        'enxCPQ__Dimension_2__c',
        'enxCPQ__Dimension_3_Numeric__c',
        'enxCPQ__Dimension_3_Value__c',
        'enxCPQ__Dimension_3__c',
        'enxCPQ__Dimension_4_Numeric__c',
        'enxCPQ__Dimension_4_Value__c',
        'enxCPQ__Dimension_4__c',
        'enxCPQ__Dimension_5_Numeric__c',
        'enxCPQ__Dimension_5_Value__c',
        'enxCPQ__Dimension_5__c',
        'enxCPQ__Hide_for_Request_Specification__c',
        'enxCPQ__Hide_for_User_Context__c',
        'enxCPQ__Hide_in_Product_Catalogue__c',
        'enxCPQ__Ignore_Inventory_Management__c',
        'enxCPQ__Ignore_Option_Requirement__c',
        'enxCPQ__Multiplier_Field__c',
        'enxCPQ__Name_CN__c',
        'enxCPQ__Name_DE__c',
        'enxCPQ__Name_EN__c',
        'enxCPQ__Name_ES__c',
        'enxCPQ__Name_FR__c',
        'enxCPQ__Name_IT__c',
        'enxCPQ__Name_PL__c',
        'enxCPQ__Pricing_Method__c',
        'enxCPQ__Product_Lifecycle_Status__c',
        'enxCPQ__Product_Lifecycle_Version__c',
        'enxCPQ__Reference_Price_Field__c',
        'enxCPQ__Row_Dimension__c',
        'enxCPQ__Row_Numeric__c',
        'enxCPQ__Row_Value__c',
        'enxCPQ__Save_Before_Calculation__c',
        'enxCPQ__Sorting_Order__c',
        'enxCPQ__TECH_Bundle_Element__c',
        'enxCPQ__TECH_Definition_Id__c',
        'enxCPQ__TECH_External_Id__c',
        'enxCPQ__TECH_Is_Configurable__c',
        'enxCPQ__TECH_Option_JSON__c',
        'enxCPQ__Unit_of_Measure__c',
        'enxCPQ__Value_From__c',
        'enxCPQ__Value_To__c'
    ]

    public static ProductResource:Array<string> = [
        'CurrencyIsoCode',
        'enxCPQ__Applicable_Solution_Variants__c',
        'enxCPQ__Assignment_Criteria__c',
        'enxCPQ__Criteria__c',
        'enxCPQ__Location_Network_Status__c',
        'enxCPQ__Location_Type__c',
        'enxCPQ__TECH_External_Id__c'
    ]

    public static Resource:Array<string> = [
        'Id',
        'Name',
        'enxCPQ__Resource_Category_Level_1__c',
        'enxCPQ__Access_Medium__c',
        'enxCPQ__Resource_Category_Level_2__c',
        'enxCPQ__Resource_Price_CAPEX__c',
        'enxCPQ__Resource_Price_MRE__c',
        'enxCPQ__Resource_Price_OTE__c',
        'enxCPQ__Resource_Price_Type__c',
        'enxCPQ__Resource_Pricing_Strategy__c',
        'enxCPQ__Resource_Type__c',
        'enxCPQ__Resource_Location_Type__c',
        'enxCPQ__TECH_External_Id__c',
        'enxCPQ__Target_Resource_Markup__c',
        'enxCPQ__Include_Resource_In_Item_Price__c',
        'ProductCode',
        'Description',
        'IsActive',
        'Family',
        'CurrencyIsoCode',
        'ExternalDataSourceId',
        'ExternalId',
        'DisplayUrl',
        'QuantityUnitOfMeasure',
        'StockKeepingUnit',
        'enxCPQ__Billing_Frequency__c',
        'enxCPQ__Bundle_Configuration_Method__c',
        'enxCPQ__Charge_Criteria__c',
        'enxCPQ__Charge_Item_Action__c',
        'enxCPQ__Charge_Model__c',
        'enxCPQ__Charge_Name__c',
        'enxCPQ__Charge_Type__c',
        'enxCPQ__Column_Dimension__c',
        'enxCPQ__Column_Numeric__c',
        'enxCPQ__Column_Value__c',
        'enxCPQ__Cover_Image_URL__c',
        'enxCPQ__Current_Inventory__c',
        'enxCPQ__Current_Lead_Time__c',
        'enxCPQ__Description_CN__c',
        'enxCPQ__Description_DE__c',
        'enxCPQ__Description_EN__c',
        'enxCPQ__Description_ES__c',
        'enxCPQ__Description_FR__c',
        'enxCPQ__Description_IT__c',
        'enxCPQ__Description_PL__c',
        'enxCPQ__Description_Pattern__c',
        'enxCPQ__Dimension_1_Numeric__c',
        'enxCPQ__Dimension_1_Value__c',
        'enxCPQ__Dimension_1__c',
        'enxCPQ__Dimension_2_Numeric__c',
        'enxCPQ__Dimension_2_Value__c',
        'enxCPQ__Dimension_2__c',
        'enxCPQ__Dimension_3_Numeric__c',
        'enxCPQ__Dimension_3_Value__c',
        'enxCPQ__Dimension_3__c',
        'enxCPQ__Dimension_4_Numeric__c',
        'enxCPQ__Dimension_4_Value__c',
        'enxCPQ__Dimension_4__c',
        'enxCPQ__Dimension_5_Numeric__c',
        'enxCPQ__Dimension_5_Value__c',
        'enxCPQ__Dimension_5__c',
        'enxCPQ__Hide_for_User_Context__c',
        'enxCPQ__Hide_in_Product_Catalogue__c',
        'enxCPQ__Ignore_Inventory_Management__c',
        'enxCPQ__Ignore_Option_Requirement__c',
        'enxCPQ__Multiplier_Field__c',
        'enxCPQ__Name_CN__c',
        'enxCPQ__Name_DE__c',
        'enxCPQ__Name_EN__c',
        'enxCPQ__Name_ES__c',
        'enxCPQ__Name_FR__c',
        'enxCPQ__Name_IT__c',
        'enxCPQ__Name_PL__c',
        'enxCPQ__Pricing_Method__c',
        'enxCPQ__Product_Lifecycle_Status__c',
        'enxCPQ__Product_Lifecycle_Version__c',
        'enxCPQ__Reference_Price_Field__c',
        'enxCPQ__Row_Dimension__c',
        'enxCPQ__Row_Numeric__c',
        'enxCPQ__Row_Value__c',
        'enxCPQ__Save_Before_Calculation__c',
        'enxCPQ__Sorting_Order__c',
        'enxCPQ__TECH_Bundle_Element__c',
        'enxCPQ__TECH_Definition_Id__c',
        'enxCPQ__TECH_Is_Configurable__c',
        'enxCPQ__TECH_Option_JSON__c',
        'enxCPQ__Unit_of_Measure__c',
        'enxCPQ__Value_From__c',
        'enxCPQ__Value_To__c'
    ]

    public static Charge:Array<string> = [
      'Name',
      'enxCPQ__Charge_Editable_in_Runtime__c',
      'enxCPQ__Multiplier_Field__c',
      'enxCPQ__Charge_Name__c',
      'enxCPQ__Pricing_Method__c',
      'enxCPQ__Charge_Model__c',
      'enxCPQ__Charge_Type__c',
      'enxCPQ__TECH_External_Id__c',
      'enxCPQ__Sorting_Order__c',
      'IsActive',
      'enxCPQ__Billing_Frequency__c',
      'enxCPQ__Unit_of_Measure__c',
      'enxCPQ__Charge_Criteria__c',
      'enxCPQ__Charge_Item_Action__c',
      'enxCPQ__Reference_Price_Field__c',
      'enxCPQ__Dimension_1__c',
      'enxCPQ__Dimension_2__c',
      'enxCPQ__Dimension_3__c',
      'enxCPQ__Dimension_4__c',
      'enxCPQ__Dimension_5__c',
      'enxCPQ__Dimension_1_Numeric__c',
      'enxCPQ__Dimension_2_Numeric__c',
      'enxCPQ__Dimension_3_Numeric__c',
      'enxCPQ__Dimension_4_Numeric__c',
      'enxCPQ__Dimension_5_Numeric__c'
    ]

    public static ProductAttribute:Array<string> = [
        'Name',
        'CurrencyIsoCode',
        'enxCPQ__Active__c',
        'enxCPQ__Option_Affecting__c',
        'enxCPQ__Enabled_for_Request__c',
        'enxCPQ__Feasibility_Affecting__c',
        'enxCPQ__Order__c',
        'enxCPQ__Product_Field_to_Update__c',
        'enxCPQ__Role__c',
        'enxCPQ__TECH_External_Id__c',
        'enxCPQ__Value_Boolean__c',
        'enxCPQ__Value_Currency__c',
        'enxCPQ__Value_Date__c',
        'enxCPQ__Value_Number__c',
        'enxCPQ__Value_Percent__c',
        'enxCPQ__Value_Text_Long__c',
        'enxCPQ__Value_Text_Short__c',
    ]

    public static AttributeValue:Array<string> = [
        'Name',
        'CurrencyIsoCode',
        'enxCPQ__Active__c',
        'enxCPQ__Decimal_Places__c',
        'enxCPQ__Exclusive_for_Products__c',
        'enxCPQ__Manual_Attribute_Value_Entry__c',
        'enxCPQ__Manual_Entry_Attribute_Type__c',
        'enxCPQ__Name_CN__c',
        'enxCPQ__Name_DE__c',
        'enxCPQ__Name_EN__c',
        'enxCPQ__Name_ES__c',
        'enxCPQ__Name_FR__c',
        'enxCPQ__Name_IT__c',
        'enxCPQ__Name_PL__c',
        'enxCPQ__Order__c',
        'enxCPQ__TECH_Definition_Id__c',
        'enxCPQ__TECH_External_Id__c',
        'enxB2B__Demarc_Config__c'
    ]

    public static AttributeRule:Array<string> = [
        'CurrencyIsoCode',
        'enxCPQ__Active__c',
        'enxCPQ__Error_Message__c',
        'enxCPQ__Order__c',
        'enxCPQ__Regexp__c',
        'enxCPQ__Rule_Attribute_Update_Logic__c',
        'enxCPQ__Rule_Criteria__c',
        'enxCPQ__TECH_External_Id__c',
        'enxCPQ__Validation_Type__c',
        'enxCPQ__Value_From__c',
        'enxCPQ__Value_To__c'
    ]

    public static Category:Array<string> = [
        'Name',
        'CurrencyIsoCode',
        'enxCPQ__TECH_Definition_Id__c',
        'enxCPQ__TECH_External_Id__c'
    ]

    public static AttributeSet:Array<string> = [
        'Name',
        'CurrencyIsoCode',
        'enxCPQ__Description__c',
        'enxCPQ__TECH_Definition_Id__c',
        'enxCPQ__TECH_External_Id__c'
    ]

    public static ProvisioningPlan:Array<string> = [
        'Name',
        'CurrencyIsoCode',
        'enxB2B__Support_Plan__c',
        'enxB2B__TECH_External_Id__c'
    ]

    public static ProvisioningTask:Array<string> = [
        'Name',
        'OwnerId',
        'CurrencyIsoCode',
        'enxB2B__Apex_handler_reference__c',
        'enxB2B__Automated_Task_Type__c',
        'enxB2B__Description__c',
        'enxB2B__Order__c',
        'enxB2B__Status__c',
        'enxB2B__TECH_External_Id__c',
        'enxB2B__Type__c'
    ]

    public static ProvisioningPlanAssignment:Array<string> = [
        'CurrencyIsoCode',
        'enxB2B__Active__c',
        'enxB2B__Criteria__c',
        'enxB2B__Item_Action__c',
        'enxB2B__Order__c',
        'enxB2B__TECH_External_ID__c'
    ]

    public static ProvisioningTaskAssignment:Array<string> = [
        'CurrencyIsoCode',
        'enxB2B__Criteria__c',
        'enxB2B__Order__c',
        'enxB2B__Predecessors__c',
        'enxB2B__Status__c',
        'enxB2B__TECH_External_ID__c'
    ]

    public static BundleElement:Array<string> = [
        'Name',
        'CurrencyIsoCode',
        'enxCPQ__Order__c',
        'enxCPQ__Selection_Max_Number__c',
        'enxCPQ__Selection_Min_Number__c',
        'enxCPQ__Selection_Type__c',
        'enxCPQ__TECH_External_Id__c'
    ]

    public static BundleElementOption:Array<string> = [
        'Name',
        'CurrencyIsoCode',
        'enxCPQ__Default__c',
        'enxCPQ__Order__c',
        'enxCPQ__TECH_External_Id__c'
    ]

    public static Pricebook:Array<string> = [
        'Name',
        'CurrencyIsoCode',
        'IsActive',
        'Description',
        'enxCPQ__Global_Partner_Price_Book__c',
        'enxCPQ__Master__c',
        'enxCPQ__Opportunity_Assignment_Criteria__c',
        // @TO-DO - handle field that were added during app lifecycle, below field was added in 7.0 and may crash with older versions
        'enxCPQ__Cart_Assignment_Criteria__c',
        'enxCPQ__Order_Assignment_Criteria__c',
        'enxCPQ__Order__c',
        'enxCPQ__Reference_Master_field__c',
        'enxCPQ__TECH_External_Id__c',
        'enxCPQ__Use_UnitPrice__c',
        'enxCPQ__Valid_From__c',
        'enxCPQ__Valid_To__c'
    ]

    public static PricebookEntry:Array<string> = [
        'UnitPrice',
        'IsActive',
        'UseStandardPrice',
        'enxCPQ__OTC_List__c',
        'enxCPQ__MRC_List__c',
        'enxCPQ__Charge_List_Price__c',
        'enxCPQ__Multiplier_Percent__c',
        'enxCPQ__Price_Modifier_Amount__c',
        'enxCPQ__Price_Modifier_Percent__c',
        'enxCPQ__Price_Override__c',
        'enxCPQ__TECH_External_Id__c',
        'enxB2B__Service_Capex__c'
    ]

    public static AttributeSetAttribute:Array<string> = [
        'Name',
        'CurrencyIsoCode',
        'enxCPQ__Order__c',
        'enxCPQ__TECH_External_Id__c'
    ]

    public static Attribute:Array<string> = [
        'Name',
        'CurrencyIsoCode',
        'enxCPQ__Active__c',
        'enxCPQ__Decimal_Places__c',
        'enxCPQ__Description_CN__c',
        'enxCPQ__Description_DE__c',
        'enxCPQ__Description_EN__c',
        'enxCPQ__Description_ES__c',
        'enxCPQ__Description_FR__c',
        'enxCPQ__Description_IT__c',
        'enxCPQ__Description_PL__c',
        'enxCPQ__Description__c',
        'enxCPQ__Disable_Create_New_Location__c',
        'enxCPQ__Display_Disabled__c',
        'enxCPQ__Display_in_Configurator__c',
        'enxCPQ__Display_not_for_Item_Action__c',
        'enxCPQ__Display_not_for_Profile__c',
        'enxCPQ__Display_not_on_Quote_Stage__c',
        'enxCPQ__Display_on_Configuration_Description__c',
        'enxCPQ__Editable_not_for_Item_Action__c',
        'enxCPQ__Editable_not_for_Profile__c',
        'enxCPQ__Editable_not_on_Quote_Stage__c',
        'enxCPQ__Helptext__c',
        'enxCPQ__Is_Propagable__c',
        'enxCPQ__Item_Field_Type__c',
        'enxCPQ__Location_Filter__c',
        'enxCPQ__Location_Type__c',
        'enxCPQ__Lookup_Field_Query__c',
        'enxCPQ__Lookup_Field__c',
        'enxCPQ__Lookup_Filters__c',
        'enxCPQ__Lookup_Object__c',
        'enxCPQ__Name_CN__c',
        'enxCPQ__Name_DE__c',
        'enxCPQ__Name_EN__c',
        'enxCPQ__Name_ES__c',
        'enxCPQ__Name_FR__c',
        'enxCPQ__Name_IT__c',
        'enxCPQ__Name_PL__c',
        'enxCPQ__Product_Field_to_Update__c',
        'enxCPQ__Required__c',
        'enxCPQ__Required_on_Quote_Stage__c',
        'enxCPQ__Source_Field_Cart__c',
        'enxCPQ__Source_Field__c',
        'enxCPQ__TECH_Definition_Id__c',
        'enxCPQ__TECH_External_Id__c',
        'enxCPQ__Type__c',
        'enxB2B__Enable_POP_selection__c',
        'enxB2B__Location_Type__c',
        'enxCPQ__Display_Turn_On_Manual_Entry_button__c'
    ]

    public static AttributeDefaultValue:Array<string> = [
        'CurrencyIsoCode',
        'enxCPQ__Attribute_Value_Text__c',
        'enxCPQ__Source_Field_Cart__c',
        'enxCPQ__Source_Field__c',
        'enxCPQ__TECH_External_Id__c',
        'enxCPQ__Value_Source__c'
    ]

    public static ProductRelationship:Array<string> = [
        'Name',
        'CurrencyIsoCode',
        'enxCPQ__Add_On_Group_Name__c',
        'enxCPQ__Max_Occurrences__c',
        'enxCPQ__Min_Occurrences__c',
        'enxCPQ__Relationship_Type__c',
        'enxCPQ__TECH_External_Id__c'
    ]

    public static AttributeValueDependency:Array<string> = [
        'CurrencyIsoCode',
        'enxCPQ__Execution_Order__c',
        'enxCPQ__TECH_Definition_Id__c',
        'enxCPQ__TECH_External_Id__c',
        'enxCPQ__TECH_Key__c',
        'enxCPQ__Unavailable_Dependent_Attribute_Value__c'
    ]

    public static PriceRule:Array<string> = [
      'Name',
      'CurrencyIsoCode',
      'enxCPQ__TECH_External_Id__c',
      'enxCPQ__Active__c',
      'enxCPQ__Conditions_Logic__c',
      'enxCPQ__Context__c',
      'enxCPQ__Discount_End_Date__c',
      'enxCPQ__Discount_Scope__c',
      'enxCPQ__Discount_Start_Date__c',
      'enxCPQ__Order__c',
      'enxCPQ__Tier_Field__c',
    ]

    public static PriceRuleCondition:Array<string> = [
      'Name',
      'CurrencyIsoCode',
      'enxCPQ__TECH_External_Id__c',
      'enxCPQ__Field_Name__c',
      'enxCPQ__Object_Name__c',
      'enxCPQ__Operator__c',
      'enxCPQ__Order__c',
      'enxCPQ__Value__c'
    ]

    public static PriceRuleAction:Array<string> = [
      'Name',
      'CurrencyIsoCode',
      'enxCPQ__Action_Type__c',
      'enxCPQ__Discount_Period__c',
      'enxCPQ__Duration__c',
      'enxCPQ__Field_Name__c',
      'enxCPQ__Order__c',
      'enxCPQ__Start_Period__c',
      'enxCPQ__Target_Field_Name__c',
      'enxCPQ__Target_Value__c',
      'enxCPQ__Tier_Value_From__c',
      'enxCPQ__Tier_Value_To__c',
      'enxCPQ__TECH_External_Id__c'
    ]

    public static WorkflowTaskDefinition:Array<string> = [
      'Name',
      'CurrencyIsoCode',
      'enxCPQ__Class_Name__c',
      'enxCPQ__Description__c',
      'enxCPQ__Flow_Name__c',
      'enxCPQ__TECH_External_Id__c',
      'enxCPQ__Default_Duration_Days__c',
      'RecordType.DeveloperName'
    ]

    public static WorkflowPlan:Array<string> = [
      'Name',
      'CurrencyIsoCode',
      'enxCPQ__Criteria__c',
      'enxCPQ__Assignment_Criteria__c',
      'enxCPQ__Order__c',
      'enxCPQ__Related_Object__c',
      'enxCPQ__TECH_External_Id__c',
      'enxCPQ__Active__c'
    ]

    public static WorkflowItem:Array<string> = [
      'CurrencyIsoCode',
      'enxCPQ__Criteria__c',
      'enxCPQ__Assignment_Criteria__c',
      'enxCPQ__Order__c',
      'enxCPQ__TECH_External_Id__c'
    ]

    public static WorkflowItemRule:Array<string> = [
      'CurrencyIsoCode',
      'enxCPQ__TECH_External_Id__c',
      'enxCPQ__Type__c'
    ]
}
